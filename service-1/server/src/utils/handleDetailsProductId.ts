...
export const handleDetailsProductId: RequestHandler<
  ParamTypes,
  unknown,
  unknown,
  QueryTypes
> = async (req, res) => {
  try {
    ...
    } = await access.getAll(req.params.productId);
    ...
    const descriptorArr = filteredHelperReduce(descriptors, /^(?!product_description)/g);
    const materialArr = filteredHelperMap(materialSpecs);
    const technicalArr = filteredHelperReduce(technicalDetails, /^(?!model_size)/g, true);
